...
def load_ostatki_sklad(wsdl_client, cursor):
    ...
    for row_firma in rows_firma:
        ...
        for row_sklad in rows_sklad:
            ...
            header = wsdl_client.header_type(document_type=1, firma=row_firma['idartmarket'].strip(),
                                             sklad=row_sklad['idartmarket'].strip())
            row_list = []
            tovar_list = []
            for r in row:
                if r['ostatok'] <= 0:
            ...
            n = wsdl_client.client.service.load_ostatki_tovar(document)
